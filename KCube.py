...
class KinesisMotor:
    ...
    def __init__(self, devicename: str, serial_no: str, homing: bool=True, pol_time: int=100, verbose: bool=True):
        ...
        if serial_no is not None:
            ...
            if self.verbose: print ("Serial is", self.serial_no)
            # create device
    ...
    def move_to(self, position: float, tolerance: float=0.02):
        ...
        '''
        try:
            if abs(self.get_pos() - position) <= tolerance:
                if self.verbose:
                    print(f'Motor already at {self.get_pos()}')
                return
            elif self.verbose:
    ...
    def resetConnection(self):
        ...
        try:
            ...
            raise Exception('No device connected') from e
